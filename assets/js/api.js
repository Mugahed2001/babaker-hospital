...
(function (w) {
  ...
  function cleanContent(html) {
    ...
    // إزالة صور الزخرفة القديمة من منشئ الصفحات (فواصل رفيعة فارغة)
    s = s.replace(/<img[^>]*\/(?:bg11|divider)[^"'\s>]*[^>]*>/gi, "");
    // إزالة فقرة لا تحوي سوى صورة زخرفية أُزيلت
    s = s.replace(/<p>\s*(?:<[^>]+>\s*)*<\/p>/g, "");
    // توحيد المحاذاة: إلغاء توسيط منشئ الصفحات ليتبع اتجاه RTL
    s = s.replace(/\s*text-align:\s*center;?/gi, "");
    return s;
  }
